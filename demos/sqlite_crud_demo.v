...
fn main() {
	...
	mut db := sqlite.connect(db_path) or {
		panic('Failed to connect to SQLite: ' + err.msg())
	}
	...
	sql db {
		...
		panic('Failed to initialize database tables: ' + err.msg())
	}
	...
	if initial_count == 0 {
		...
		p1 := Product{ name: 'MacBook Pro M3', category: 'Electronics', stock: 45, price: 1999.99, in_stock: true, date_recv: '2026-07-01' }
		p2 := Product{ name: 'Ergonomic Desk Chair', category: 'Furniture', stock: 15, price: 299.50, in_stock: true, date_recv: '2026-06-15' }
		p3 := Product{ name: 'Wireless Bluetooth Mouse', category: 'Electronics', stock: 120, price: 49.99, in_stock: true, date_recv: '2026-07-02' }
		p4 := Product{ name: 'Classic Leather Notebook', category: 'Books', stock: 80, price: 18.00, in_stock: true, date_recv: '2026-06-30' }
		p5 := Product{ name: 'Ultra-light Running Shoes', category: 'Apparel', stock: 0, price: 89.90, in_stock: false, date_recv: '2026-05-20' }
		...
		sql db {
			...
			panic('Failed to seed database: ' + err.msg())
		}
	}
	...
	win.on_click('save_row_btn', fn [mut state] (mut w &simplegui.SimpleWindow) {
		...
		sql state.db {
			update Product set name = name, category = category, stock = stock, price = price, in_stock = in_stock, date_recv = date_recv where id == prod_id
		} or {
	...
	win.on_click('delete_row_btn', fn [mut state] (mut w &simplegui.SimpleWindow) {
		...
		if w.confirm('Confirm Deletion', 'Are you sure you want to permanently delete product "' + prod.name + '" from the database?') {

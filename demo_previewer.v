...
fn main() {
	...
	win.on_change('tpl_picker', fn [mut state] (mut w simplegui.SimpleWindow, tpl_name string) {
		...
		if snippet != '' {
			w.set_text('code_editor', snippet)
	...
	if capture_path != '' {
		win.after(1000, fn [capture_path] (mut w simplegui.SimpleWindow) {
			w.dispatch_event('tpl_picker', 'change', 'Minimal Window')
			w.after(1000, fn [capture_path] (mut w2 simplegui.SimpleWindow) {
				w2.capture_screenshot(capture_path)
				w2.after(400, fn (mut w3 simplegui.SimpleWindow) {
					w3.close()
				})
			})
		})

...
| **📝 Text Editor Pro** | [`applications/text_editor.v`](file:///Users/codecaine/vlang_simplegui/applications/text_editor.v) | Full-featured native code editor & document workspace: multi-font monospace/sans typography (Menlo, SF Mono, Monaco, Courier New, Helvetica), dynamic font scaler, case converters (UPPER, lower, Title Case), line sorter (A-Z), line deduplicator, whitespace cleaner, JSON prettifier, Base64 encoder/decoder, search & case-insensitive replace all, dirty state tracking, and line/word/character telemetry. |
